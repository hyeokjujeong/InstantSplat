...
def check_single_object(obj_dict, temp_corr, n):
    import copy
    np1 = n+1
    result = []
    for temp in temp_corr:
...
def get_temp_mask(mask_list, idx, obj_list_temp):
    pred_mask = np.zeros(mask_list[idx].shape[:2], dtype=np.uint8)
    for obje in obj_list_temp:
        masking_pred = (mask_list[idx]==obje)
...
def main(data_dir, video_dir,n_view, px, py):
    ...
    for test_img in test_files:
        ...
        gt_mask_list = get_gt_masks(gt_names, px, py)
        ...
        for i in obj_dict[f'{len(image_files)-1}']:
            masking_pred = (mask_list[-1] == i)
            pred_mask[masking_pred] = 1
        pred_mask_img = Image.fromarray(pred_mask*255)
        os.makedirs(video_dir+'/mask', exist_ok=True)
