...
from utils.sfm_utils import (save_intrinsics, save_extrinsic, save_points3D, save_time, save_images_and_masks,
                             init_filestructure, get_sorted_image_files, split_train_test, load_images, compute_co_vis_masks, load_images_single_channel)
...
def main(source_path, model_path, ckpt_path, device, batch_size, image_size, schedule, lr, niter, 
    ...
    obj_list = get_object_masks(mask_list, output, model)
    ...
    for i in range(len(obj_list)):
        for j in range(len(mask_list)):
            if str(j) in obj_list[i]:
                for k in obj_list[i][str(j)]:
                    final_masks[j, mask_list[j]==k]= i
    ...
    pts_num = save_points3D(sparse_0_path, imgs, pts3d, confs.reshape(pts3d.shape[0], -1), overlapping_masks, use_masks=co_vis_dsp, save_all_pts=True, save_txt_path=model_path, depth_threshold=depth_thre, mask_3d=final_masks)
